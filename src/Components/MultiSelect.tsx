...
interface MultiSelectProps {
  options: string[];
  onSelectionChange: (selectedOptions: string[]) => void;val:string[];
}

const MultiSelect: React.FC<MultiSelectProps> = ({ options, onSelectionChange,val }) => {
  ...
  const handleCheckboxChange = (option: string) => {
    ...
    if (option === 'Self-describe') {
      updatedSelectedOptions = selectedOptions.includes(selfDescribe)
        ? selectedOptions.filter(selectedOption => selectedOption !== selfDescribe)
  ...
  const handleSelfDescribeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      {options.map(option => (
        <div key={option} className='mt-2 text-[14px]'>
          <label>
            <input
              type="checkbox"
              checked={selectedOptions.includes(option === 'Self-describe' ? selfDescribe : option)}
              onChange={() => handleCheckboxChange(option)}
              className='bg-red-500 mr-2'
            />
            {option}
          </label><br/>
          {option === 'Self-describe' && selectedOptions.includes(selfDescribe) && (
            <input
