...
const areArraysEqual = (arr1: MultiValue<Option>, arr2: MultiValue<Option>) => {
...
const SelectTags: React.FC<Props> = ({ options, phdr = 'Select Options', handle, val }) => {
  ...
  useEffect(() => {
    if (flg === 1) {
      const valuesArray = selectedOptions.map(obj => obj.value);
      console.log(valuesArray);
      handle(valuesArray);
      setFlg(0);
    }
  }, [selectedOptions, flg, handle]);

  useEffect(() => {
    if (!areArraysEqual(val, selectedOptions)) {
      const objectArray = Array.isArray(val)
        ? val.map(item => ({
            value: item.value,
            label: item.label
          }))
