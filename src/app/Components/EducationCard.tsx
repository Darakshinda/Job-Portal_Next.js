...
import SearchableSelect from './SearchableSelect';import degreeOpns from '../profile/edit/data/degree.json';

interface EducationCardProps {
  education: string;
  graduation: Date | null;
  gpa: string;maxgpa: string;
  degree: string;ind:number;EdusAtInd:Function;
  update:Function;del:Function;flgedit:boolean;
}

const EducationCard: React.FC<EducationCardProps> = ({ education, degree, graduation=null, gpa, maxgpa,ind,update,EdusAtInd,flgedit,del }) => {
    ...
    const handle = (key: string, value: string,obj:object,setObj:Function) => {
      ...
      const [edudef, setedudef] = useState({
        education: education,degree:degree,graduation:graduation,gpa:gpa,maxgpa:maxgpa,
      });
...
return (
    ...
    {!edit&&<div className="border rounded-lg p-4 shadow-md flex justify-between items-start hover:border-purple-400 bg-black mt-[9px]">
      <div className="flex items-start">
        ...
        <div>
          ...
          {gpa&&<div className="text-white">{gpa}{maxgpa!=''&&<a>/{maxgpa}</a>} GPA</div>}
    ...
    <button className="text-white font-bold py-2 px-8 rounded" style={{backgroundColor:buttonbg}} onClick={(e)=>{update(ind,edu);flgedit(true);setedudef(edu);
    setedit(false);}}>
      Cancel
    </button>
    <button className="bg-purple-500 text-white font-bold px-8 rounded" style={{backgroundColor:buttonbg}} 
    onClick={(e)=>{update(ind,edu);flgedit(true);setedudef(edu);
    setedit(false);}}>
      Save
    </button>
    <button className="text-white text-[12px] ml-[11px] font-bold"
     onClick=
     {(e)=>{if (window.confirm('Are you sure you want to delete this Education?'))
            {del(ind);flgedit(false);console.log(EdusAtInd(ind+1));setedudef(EdusAtInd(ind+1));setedu(EdusAtInd(ind+1));
                setedit(false);}}}>
      Remove Education
    </button>
  </div>
